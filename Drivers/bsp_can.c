...
void CAN1_QuickInit(void){
	...
	RCC_AHB1PeriphClockCmd(CAN1_RX_GPIO_CLK | CAN1_TX_GPIO_CLK, ENABLE);    /* 使能 GPIO 时钟 */
	...
	GPIO_PinAFConfig(CAN1_TX_GPIO_PORT, CAN1_TX_PINSOURCE, GPIO_AF_CAN1); 	/*  连接 IO 到 CAN1_Tx*/
	GPIO_PinAFConfig(CAN1_RX_GPIO_PORT, CAN1_RX_PINSOURCE, GPIO_AF_CAN1); 	/*  连接 IO 到 CAN1_Rx*/
	...
	CAN_InitStructure.CAN_SJW = CAN1_SJW;		   //BTR-SJW 重新同步跳跃宽度2个时间单元
	CAN_InitStructure.CAN_BS1 = CAN1_BS1;		   //BTR-TS1 时间段1 占用了4个时间单元
	CAN_InitStructure.CAN_BS2 = CAN1_BS2;		   //BTR-TS1 时间段2 占用了6个时间单元	
...
/**
  * @brief  CAN发送数据
  * @param  CANx 		  CAN编号
  * 	    	id_type ·	id类型 CAN_ID_STD（标准）， CAN_ID_EXT（扩展）
  *			    id			  id号
  * 	    	data[8]		8个数据
  * @retval None
  */
